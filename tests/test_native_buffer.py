...
def test_enqueue_video_yuy2_native_buffer_invalid_type(video_player):
    """native_buffer に無効な型を渡した場合のテスト."""
    # 文字列を渡すとエラー (PyCapsule でないため ValueError)
    with pytest.raises(ValueError):
        video_player.enqueue_video_yuy2("invalid", 0)

    # 整数を渡すとエラー
    with pytest.raises(ValueError):
        video_player.enqueue_video_yuy2(12345, 0)
...
def test_enqueue_video_yuy2_native_buffer_wrong_capsule_name(video_player):
    """間違った name の PyCapsule を渡した場合のテスト."""
    pythonapi = ctypes.pythonapi
    pythonapi.PyCapsule_New.argtypes = [
        ctypes.c_void_p,
    ...
    with pytest.raises(ValueError):
        video_player.enqueue_video_yuy2(wrong_name_capsule, 0)
...
def test_enqueue_video_yuy2_native_buffer_no_name_capsule(video_player):
    """name が None の PyCapsule を渡した場合のテスト."""
    pythonapi = ctypes.pythonapi
    pythonapi.PyCapsule_New.argtypes = [
        ctypes.c_void_p,
    ...
    with pytest.raises(ValueError):
        video_player.enqueue_video_yuy2(no_name_capsule, 0)
